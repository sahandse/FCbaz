...
class ChemistryEngineFC27 {
  ...
  ChemistryResult calculate({
    ...
    final players = playersBySlot.values.toList();
    ...
    for (final slot in formation.slots) {
      ...
      final inPosition = _isInPosition(player, slot.position);
      if (!inPosition) {
        bySlot[slot.id] = 0;
        continue;
      }
      ...
      final club = _weightedClubCount(player, players);
      final league = _weightedLeagueCount(player, players);
      final nation = _weightedNationCount(player, players);
  ...
  bool _isInPosition(Player player, String slotPosition) {
    return player.position == slotPosition || player.positions.contains(slotPosition);
  }
  ...
  int _weightedClubCount(Player target, List<Player> players) {
    return players.where((p) => p.clubName.isNotEmpty && p.clubName == target.clubName).length;
  }

  int _weightedLeagueCount(Player target, List<Player> players) {
    var count = players.where((p) => p.leagueName.isNotEmpty && p.leagueName == target.leagueName).length;

    for (final player in players) {
      if (_isIcon(player)) {
        count += 1;
      } else if ((_isHero(player) || _isHallOfFut(player)) &&
          player.leagueName == target.leagueName) {
        count += 1;
      }
    }
  ...
  int _weightedNationCount(Player target, List<Player> players) {
    var count = players.where((p) => p.nationName.isNotEmpty && p.nationName == target.nationName).length;

    for (final player in players) {
      if ((_isIcon(player) || _isHero(player) || _isHallOfFut(player)) &&
          player.nationName == target.nationName) {
        count += 1;
      }
    }
